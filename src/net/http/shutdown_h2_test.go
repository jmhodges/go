...
// Issue XXXXX: Shutdown polls until its context expires if an HTTP/2
// connection finishes its TLS handshake after Shutdown has run the
// http2 server's one-shot GOAWAY sweep (the OnShutdown hook registered
// by configureHTTP2). Such a connection registers with the http2 server
// after the sweep, so it is never sent a GOAWAY, and closeIdleConns
// never reclaims it because HTTP/2 connections are parked in
// StateActive for their whole lifetime (see issue 39776).
//
// The server's GetCertificate callback stalls the handshake until after
// Shutdown's sweep has run, standing in for the race window between
// conn.serve's TLS handshake and http2.Server.serveConn's registerConn.
...
func testServerShutdownH2ConnRegisteredDuringShutdown(t *testing.T) {
	...
	// Start a TLS handshake; it stalls in GetCertificate.
	...
	<-inHandshake // conn is accepted and mid-handshake: not yet registered with the http2 server
